...
public class PatenteService extends AsyncTask<String, Void, BaseDTO> {
	...
	protected BaseDTO doInBackground(String... args) {
		try {
			Document doc = DocumentBuilderFactory.newInstance().newDocumentBuilder().parse(PATENTE_URL + args[0] + "%23" + args[1] + "%23" + (int) Math.pow(args[0].length() + args[1].length(), 2D) % 93 + "%2325");
			return new PatenteDTO(doc.getElementsByTagName(NUMEO_PATENTE).item(0).getTextContent(), doc.getElementsByTagName(SALDO).item(0).getTextContent(), doc.getElementsByTagName(SCADENZA_PATENTE).item(0).getTextContent());
		} catch (Exception e) {
			e.printStackTrace();
